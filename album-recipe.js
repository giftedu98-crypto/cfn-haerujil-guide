(() => {
  ...
  const openDetail=item=>{
    ...
    document.querySelector('#dexDetailName').textContent=name;document.querySelector('#dexDetailPhoto').src=photo||'';document.querySelector('#dexDetailPhoto').style.display=photo?'block':'none';document.querySelector('#dexDetailDescription').textContent=description(name);document.querySelector('#dexDetailRules').innerHTML='<div><span>금지체장</span><b>'+escape(rule[0])+'</b></div><div><span>금어기</span><b>'+escape(rule[1])+'</b></div>';document.querySelector('#dexDetailRecipe').innerHTML='<h3>'+(edible?'추천 조리법':'안전 안내')+'</h3><p>'+escape(recipe)+'</p>'+(edible?'<a class="recipe-search" href="'+search+'" target="_blank" rel="noopener">인터넷 조리법 더 보기 ↗</a>':'');detail.classList.add('show');
  };
